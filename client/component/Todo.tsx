...
export const Todo = ({ todo }: TodoProps) => {
  ...
  const handleEdit = async() => {
        ...
        `${API_URL}/editTodo/${todo.id}`,
    {
      ...
      if (response.ok) {
        ...
        mutate(todos.map((item) => (item.id === todo.id ? editedTodo : item)))
      }
    }
  };
  ...
  const handleDelete = async() => {
    ...
    if (response.ok) {
      ...
      mutate(todos.map((item) => (item.id === todos.id ? deletedTodo : item)))
    }
  };

  const toggleTodoCompletion = async(id: number, isCompleted) => {
    const response = await fetch(
      `${API_URL}/editTodo/${todo.id}`,
    {
      method: "PUT",
    ...
    if (response.ok) {
      const editedTodo = await response.json();
      mutate(todos.map((item) => (item.id === todos.id ? editedTodo : item)))
    }
  }
